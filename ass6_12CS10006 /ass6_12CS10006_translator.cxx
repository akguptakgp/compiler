	...
	else if(arg1!=0) //Unary Operator
	{
		switch(op)
		{
			...
			case ASSIGN : out<<result<<"="<<arg1<<endl;;       //  printf("%s=%s\n",result,arg1);
				   if(arg1[0]=='\"')  {
				   			 char *buffer=new char[20]; sprintf(buffer,".LC%d",string_counter++);
				   			string_table=merge(string_table,makelist(arg1,buffer));}
			break;
...
void convBool2Int(expression& e)
{
    ...
    sprintf(buffer,"%d",nextinstr()+PC+2); 
