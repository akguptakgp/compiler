			...
			|postfix_expression P22 expression P23  //Arrays in Expressions 
					{
					  ...
					  $$.type=$1.symptr->type->right;
                                          $$.symptr=ST_gl.gentemp(new Type_Data(Int),NULL);
...
direct_declarator:
		   ...
		   |direct_declarator P22 type_qualifier_list_opt assignment_expression_opt P23 // Array Declaration
				{
					$$=$1;	
                                        ST_funt->update(new Type_Data(Array,atoi($4.symptr->initial_value),NULL),$1);
                                }	
...
identifier_list:           // to be done // this rule will never get executed  becuase parameter declaration is not allowed inside a function 						//and a function can only be called from a function
